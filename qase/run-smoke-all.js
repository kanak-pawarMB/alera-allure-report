/**
 * Qase Integration Runner — Smoke Suite
 * =======================================
 * Supports running a single spec file OR all smoke test files.
 *
 * Workflow:
 *   1. Scan smoke spec file(s) for ONEVIEW case IDs
 ...
 *   node qase/run-smoke-all.js
 *   npm run test:smoke-qase
 *
 *   # Run a single smoke file:
 *   node qase/run-smoke-all.js ADTAlerts
 *   node qase/run-smoke-all.js Login
 *   node qase/run-smoke-all.js Demographics
 *
...
function resolveTargetFiles(fileArg) {
  const allFiles = readdirSync(SMOKE_DIR)
    .filter(f => f.endsWith('.spec.js'))
    .sort();

  if (fileArg) {
    // Normalise: allow "ADTAlerts" or "ADTAlerts.spec.js"
    ...
    if (!match) {
      ...
      console.error(`   Available smoke files:\n${allFiles.map(f => `     ${f}`).join('\n')}`);
      process.exit(1);
    }
...
function extractCaseIds(specFiles) {
  ...
  for (const file of specFiles) {
    const content = readFileSync(join(SMOKE_DIR, file), 'utf8');
...
// ── Step 5: Parse results from JSON output ───────────────────────────────────
...
async function main() {
  ...
  // Optional file argument: node qase/run-smoke-all.js ADTAlerts
  ...
  const label = isSingleFile
    ? `${targetFiles[0].replace('.spec.js', '')} Smoke`
    : 'Smoke Suite';

  const runTitle = `${label} – ${date}`;
  ...
  if (isSingleFile) {
    ...
    console.log(`\n📂 Scanned ${targetFiles.length} smoke spec file(s):`);
  ...
  if (uniqueIds.length === 0) {
    console.warn('\n⚠️  No ONEVIEW case IDs found in smoke spec files. Nothing to report to Qase.');
    process.exit(0);
  }
  ...
  try {
    ...
    for (const file of targetFiles) {
      ...
      const testPath = `tests/smoke/${file}`;
      runPlaywrightTests(testPath);

      // 4. Parse results for this file and merge

...
test.describe('Microsoft Login Setup — Save session', () => {
  ...
  test('login and save session', async ({ page, browserName }) => {
    // Skip for Firefox and Safari - MFA triggers on these browsers
    // auth.json saved from Chromium works across all browsers
    test.skip(browserName !== 'chromium', 'Auth setup runs only on Chromium to avoid MFA issues');
